"""Synthetic catchments, for tests and for CI.

The real network is compiled from OpenStreetMap, which means it needs a network call
and cannot be committed (data/artifacts is git-ignored, and rebuildability is the
point - GC-6). Everything downstream of the compiler still needs *a* CompiledNetwork
to test against, so these builders produce deterministic ones offline.

`toy_gdfs` is the six-node network the compiler's own unit tests reason about by hand.
`line_network_gdfs` reaches PRD scale (40 entry sources) for latency work without
needing the real catchment.
"""
...
def toy_gdfs():
    ...
    nodes = gpd.GeoDataFrame(
        {"node_id": list(pts),
         "node_type": ["outfall", "outfall", "junction", "reach_point", "reach_point",
                       "reach_point"],
    ...
    zones = gpd.GeoDataFrame(
        {"zone_id": ["ZONE_A", "ZONE_B"], "node_id": ["ZA", "ZB"],
         ...
         "geometry": [_box(pts["ZA"], 0.002), _box(pts["ZB"], 0.002)]}, crs="EPSG:4326")
...
def line_network_gdfs(n_nodes: int = 400, n_entries: int = 40):
    ...
    zones = gpd.GeoDataFrame(
        {"zone_id": [f"ZONE_{i}" for i in range(3)], "node_id": zone_nodes,
         ...
         "geometry": [_box(coords[n], 0.0002) for n in zone_nodes]}, crs="EPSG:4326")
    return nodes, edges, outfalls, zones


SYNTHETIC_BUILDERS = {"toy": toy_gdfs, "line": line_network_gdfs}
